...
namespace GradationBaker.UI
{
    ...
    internal static class DennokoUIFont
    {
        ...
        // 前半は dennokoworks 共通セット、後半は Localization/ja.json から抽出した
        // Gradation Baker 固有の文字（重複除去済み）。ja.json に文言を足したら追記する。
        private const string WarmupJapanese =
            "適用保存解除追加削除設定選択中対象有効無効表示非切替更新確認取消閉開始了" +
            "はいいえ完了失敗警告情報エラー成功準備実行処理読込書出" +
            "グデションテクスチャジェネレタツル化です。して編集くださメッュこにをド＆ロプべリ" +
            "アベイる作業生破壊ケコポト使時の座標ズ防止カボ制御合わせビハ調整•回転方向変移動" +
            "赤緑範囲心高サ形状ニ一フィ球放射セミ軸なたま重り明優先暗力ォ像度マダ背景色透白黒" +
            "拡張境界、ム軽減モキ頂点反ごと分割ソ言語待機枚・ヒから必要応じ各「」※やギがれ場" +
            "試展以下可能指よ複避個別停本終再外自ん置換算乗ザ強あ最版得き";
